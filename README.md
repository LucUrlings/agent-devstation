...
After a container restart or recreation, run `codex remote-control start` again; the image does not start Remote Control until you choose to. Your ChatGPT login remains in the home volume. A recreated container downloads Codex's separate daemon package again on first use. Startup clears stale daemon process records and sockets left in the home volume by the previous container. If the official CLI times out waiting for the socket during startup, the image retries this one command twice; other Codex commands run directly.
