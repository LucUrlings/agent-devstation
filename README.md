...
The seccomp file is based on [Docker's default profile](https://github.com/moby/profiles/blob/main/seccomp/default.json). It additionally permits `clone` and `unshare` when creating a user namespace, plus `mount`, `pivot_root`, and `umount2`, so Codex's bubblewrap sandbox can run inside Docker. It does **not** make the container privileged or disable seccomp, but it widens the container's syscall access. Keep the file paired with Compose when copying or updating the deployment. See [NOTICE](NOTICE) for source and license details. [Docker's seccomp documentation](https://docs.docker.com/engine/security/seccomp/) explains these controls.
